...
"uniform vec4 wantedColor;\n"
"out vec4 FragColor;\n"
"void main()\n"
"{\n"
"   FragColor = wantedColor;\n" //vec4(1.0f, 0.5f, 0.2f, 1.0f)
"}\n\0";
...
int GraphicRenderer::renderCubes(const std::vector<RigidBody>& rbs,
	...
	fvertices.clear();
	for each  (const RigidBody& RB in rbs)
	{
		renderCubes(RB);
	}
	...
	if (!glfwWindowShouldClose(window))
	{
		...
		unsigned int colorLocation = glGetUniformLocation(shaderProgram, "wantedColor");

		//then we pass each matrix into our shader
		glUniformMatrix4fv(modelLocation, 1, GL_FALSE, &model[0][0]);
		glUniformMatrix4fv(viewLocation, 1, GL_FALSE, &view[0][0]);
		glUniformMatrix4fv(projectionLocation, 1, GL_FALSE, &projection[0][0]);
		//we give to the frag shader a color
...
void GraphicRenderer::addVectorToVertices(const Vector3D& v) {
	fvertices.push_back(static_cast<float>(v.x));
	fvertices.push_back(static_cast<float>(v.y));
	fvertices.push_back(static_cast<float>(v.z));
}
